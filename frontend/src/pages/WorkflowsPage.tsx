...
export default function WorkflowsPage({ search }: { search: string }) {
  ...
  const toggleState = (id: number) => {
    setWorkflows(prev =>
      prev.map(w => w.id === id ? { ...w, state: !w.state } : w)
    );
  };
  ...
  let filtered = workflows
    ...
    .filter(w => onlyPinned ? w.pinned : true);
  ...
  const counts = {
    ...
    active: workflows.filter(w => w.state).length,
    paused: workflows.filter(w => !w.state).length
  };
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex gap-3 text-sm">
        <button
          className={`px-3 py-1 rounded ${tab === "all" ? "bg-blue-100 text-blue-700" : "bg-gray-200"}`}
          onClick={() => setTab("all")}
        >
          All ({counts.all})
        </button>
        <button
          className={`px-3 py-1 rounded ${tab === "active" ? "bg-blue-100 text-blue-700" : "bg-gray-200"}`}
          onClick={() => setTab("active")}
        >
          Active ({counts.active})
        </button>
        <button
          className={`px-3 py-1 rounded ${tab === "paused" ? "bg-blue-100 text-blue-700" : "bg-gray-200"}`}
      ...
      <div className="flex justify-between items-center gap-3">
        <div className="flex gap-3 items-center">
          ...
          <input
            ...
            className="border px-3 py-1 rounded w-80 text-sm"
            value={filter}
              ...
              <td className="p-2">
                <label className="inline-flex relative items-center cursor-pointer">
                  <input
                    ...
                    onChange={() => toggleState(w.id)}
                  />
                  <div className="w-11 h-6 bg-gray-200 rounded-full peer peer-checked:bg-blue-500 peer-focus:ring-2 peer-focus:ring-blue-300 transition-all"></div>
